...
app.config["SQLALCHEMY_DATABASE_URI"] = database_url or "sqlite:///university.db"
...
class StudentPersonalInfo(db.Model):
    ...
    guardian_mobile = db.Column(db.String(20), nullable=True)
...
def handle_preflight():
    if request.method == "OPTIONS":
        ...
        # Allow local/dev origins dynamically instead of pinning to one port.
...
def login():
    ...
    username = (data.get("username") or data.get("uid") or "").strip()  # Accept both username and uid
    ...
    if not user.check_password(password):
        # Local/dev fallback: allow admin login using .env credentials without DB mutation.
        env_admin_user = os.getenv("ADMIN_USERNAME", "admin").strip()
        env_admin_pass = os.getenv("ADMIN_PASSWORD", "abcd1234").strip()
        is_env_admin_login = (
            user.role == "admin"
            and username == env_admin_user
            and password == env_admin_pass
        )
        if not is_env_admin_login:
            return jsonify({"error": "Invalid credentials"}), 401
...
def sanitize_payload_for_model(model_class, payload):
    """Keep only model-backed columns to prevent invalid kwargs on writes."""
        ...
        for data_key, (model_class, rel_name) in model_mappings.items():
            if data_key in data:
                ...
                if rel_name in ["personal_info", "career_objective", "skills", "swoc"]:
                    ...
                    clean_payload = sanitize_payload_for_model(model_class, rel_payload)
                    if existing_record:
                        # Update existing record
                        for field, value in clean_payload.items():
                            if hasattr(existing_record, field):
                                setattr(existing_record, field, value)
                    else:
                        # Create new record
                        new_record = model_class(**clean_payload, student_id=student.id)
                ...
                # Handle one-to-many relationships
                    ...
                    if isinstance(rel_payload, list):
                        for item in rel_payload:
                            clean_item = sanitize_payload_for_model(model_class, item)
                            new_record = model_class(**clean_item, student_id=student.id)
                            db.session.add(new_record)
...
@role_required(["student"])
def upload_my_student_photo():
    """Upload or replace current student's profile photo in Cloudinary."""
    user = db.session.get(User, get_jwt_identity())
    ...
    if not student.personal_info:
        return jsonify({"error": "Please save personal info before uploading photo"}), 400

    photo = request.files.get("photo")
    if not photo:
        return jsonify({"error": "Missing 'photo' file in request"}), 400

    if not (photo.mimetype or "").startswith("image/"):
        return jsonify({"error": "Invalid file type. Please upload an image."}), 400

    # Enforce 2MB max to match frontend guardrails.
    photo.stream.seek(0, os.SEEK_END)
    file_size = photo.stream.tell()
    photo.stream.seek(0)
    if file_size > 2 * 1024 * 1024:
        return jsonify({"error": "File too large. Max allowed size is 2MB."}), 400

    if not CLOUDINARY_READY:
        return jsonify({"error": "Cloudinary is not configured on server"}), 500

    try:
        upload_result = cloudinary.uploader.upload(
            photo,
            folder="kys/student_profiles",
            public_id=f"student_{student.uid}_profile",
            overwrite=True,
            resource_type="image",
        )
        ...
        student.personal_info.photo_url = photo_url
        student.personal_info.photo_public_id = public_id
        db.session.commit()
